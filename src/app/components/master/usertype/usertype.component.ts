...
export class UsertypeComponent implements OnInit, OnDestroy {
  ...
  getData() {
    this._httpService
      .get(environment.BASE_API_PATH + 'UserType/GetAll')
      .subscribe((res) => {
        if (res.isSuccess) {
          this.objRows = res.data;
        } else {
          this._toastrService.error(res.errors[0], 'UserType Master');
        }
      });
  }
  ...
  submitForm() {
    ...
    switch (this.dbOps) {
      case DbOperation.create:
        this._httpService
          .post(
            environment.BASE_API_PATH + 'UserType/Save/',
            this.addForm.value
          )
          .subscribe((res) => {
            if (res.isSuccess) {
              ...
              this._toastrService.error(res.errors, 'UserType Master');
            }
          });
        break;
      case DbOperation.update:
        this._httpService
          .post(
            environment.BASE_API_PATH + 'UserType/Update/',
            this.addForm.value
          )
          .subscribe((res) => {
            if (res.isSuccess) {
    ...
    swalWithBootstrapButtons
      ...
      .then((result) => {
        if (result.isConfirmed) {
          this._httpService
            .post(environment.BASE_API_PATH + 'UserType/Delete/', obj)
            .subscribe((res) => {
              if (res.isSuccess) {
